...
const HeroSection = ({ onBookAudit }: HeroSectionProps) => (
  <section className="relative pt-32 pb-20 md:pt-40 md:pb-24 px-6 flex flex-col items-center text-center w-full border-b border-border bg-background bg-[radial-gradient(hsl(214_32%_80%)_1px,transparent_1px)] [background-size:40px_40px] [background-position:center_top]">
    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-border bg-card shadow-sm mb-6">
      <span className="text-xs font-medium text-foreground tracking-widest uppercase">Enterprise AI Solutions</span>
    </div>

    <h1 className="text-4xl md:text-6xl lg:text-7xl font-normal text-foreground tracking-tight leading-[1.1] max-w-4xl mb-6 drop-shadow-sm">
      Agentic AI systems<br />for the enterprise
    </h1>

    <p className="text-lg md:text-xl max-w-2xl text-muted-foreground mb-10 leading-relaxed font-normal">
    ...
    <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl text-left">
      {heroCards.map((card) => (
        <div key={card.title} className="skeu-card p-6 group cursor-pointer flex flex-col justify-between min-h-[160px] hover:-translate-y-0.5 transition-transform duration-300">
          <div>
            <h3 className="text-xl font-medium tracking-tight text-foreground mb-2 flex items-center justify-between">
              {card.title}
              <div className="skeu-icon-badge !p-1.5 !rounded-lg group-hover:!bg-foreground group-hover:text-card transition-colors">
                <ArrowRight className="w-4 h-4" />
              </div>
            </h3>
            <p className="text-base text-muted-foreground font-normal leading-relaxed">{card.desc}</p>
          </div>
        </div>
      ))}
    </div>
